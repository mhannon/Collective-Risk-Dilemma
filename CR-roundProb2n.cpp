...
double
diffFNs(int const &probOPT, double const &prob, double t, double x, double const &gamma, double const &groupSize);

double weightedFrequency(Individual player, double freqT);

void wrightFisherPopulation(std::vector <Individual> &popN, int const &NinD, std::mt19937_64 &rng,
                            std::uniform_real_distribution<double> &uniR);

void onegame(std::vector <Individual> &popN0, std::vector <Individual> &popN1, int const &roundN, int const &inD,
             int const &gInd, double const &prob, int const &probOPT, double const &gamma, double *totalC0,
             double *totalC1, double *totalE0, double *totalE1, double *genC0, double *genC1, string outOpt,
             int const &randRound, std::mt19937_64 &rng, std::uniform_real_distribution<double> &uniR,
             std::uniform_int_distribution<int> &uni2);

void onegeneration(std::vector <Individual> &popN0, std::vector <Individual> &popN1, int const &roundN, int const &inD,
                   int const &gInd, double const &selectionCo, int const &gameNum, double const &prob,
                   int const &probOPT, double gamma, double *totalC0, double *totalC1, double *totalE0, double *totalE1,
                   double *genC0, double *genC1, string outOpt, int const &randRound, std::mt19937_64 &rng,
                   std::uniform_real_distribution<double> &uniR, std::uniform_int_distribution<int> &uni2);

void
mutatedGeneration(std::vector <Individual> &popN, std::vector<double> &mutationP, int const &inD, int const &roundN,
                  double *sigma, std::mt19937_64 &rng, std::uniform_real_distribution<double> &uniR,
                  std::uniform_int_distribution<int> &uni2);
...
int main(int argc, char *argv[]) {
    ...
    int agValue = 1;
    //RandNum	RNUm;
    string outOpt(argv[agValue]);
    agValue++;// is for output opts 1 is for total ave and 2 is a generation file
    int clusterInfo = atoi(argv[agValue]);
    agValue++;
    int inD = atoi(argv[agValue]);
    agValue++;
    double selectionCo = atof(argv[agValue]);
    agValue++;
    int gameNum = atoi(argv[agValue]);
    agValue++;
    int maxgen = atoi(argv[agValue]);
    agValue++;///for the looping to a max of genrations
    double mutation = atof(argv[agValue]);
    agValue++;
    //Game
    int gInd = atoi(argv[agValue]);
    agValue++;
    double groupSize = double(gInd) + double(gInd);
    int roundN = atoi(argv[agValue]);
    agValue++;
    double maxPay0 = atof(argv[agValue]);
    agValue++;
    double maxPay1 = atof(argv[agValue]);
    agValue++;
    double indEndowment0 = atof(argv[agValue]);
    agValue++;
    double indEndowment1 = atof(argv[agValue]);
    agValue++;
    double riskEndowment0 = atof(argv[agValue]);
    agValue++;
    double riskEndowment1 = atof(argv[agValue]);
    agValue++;
    double gameTarget = atof(argv[agValue]);
    agValue++;
    //Probability functions
    double prob = atof(argv[agValue]);
    agValue++;
    int probFNOPT = atof(argv[agValue]);
    agValue++;//this changes it from heavy side, linear to other functions
    double gamma = atof(argv[agValue]);
    agValue++;/// is the gamma value in risk probability function
    double sigma[2];
    sigma[0] = atof(argv[agValue]);
    agValue++; //gaussian mutation, -1 means doesnt exist, for threshold
    sigma[1] = atof(argv[agValue]);
    agValue++; //gaussian mutation, for contributions
    int randRound = atoi(argv[agValue]);
    agValue++; //0= all rounds, 1 one random round .... roundN
    double initContribution = atof(argv[agValue]);
    agValue++; //-1 = random 0-..... is what indiviudals will contribute per round, leads starting condition for the behavior of all individuals
    ...
    double *totalC0 = new double[((roundN) +
                                  6)];//caclulates the sum of all the columns over all generations and individuals
    double *totalE0 = new double[((roundN) +
                                  1)];//caclulates the sum of all the columns over all generations and individuals
    double *genC0 = new double[(roundN + 1)];
    double *totalC1 = new double[((roundN) +
                                  6)];//caclulates the sum of all the columns over all generations and individuals
    double *totalE1 = new double[((roundN) +
                                  1)];//caclulates the sum of all the columns over all generations and individuals
    ...
    if (inD > 0) {
        for (int i2 = 0; i2 < mutationProb0.size(); i2++) {
            if (flag == 1) {
                ...
                if (((inD >= 100) && ((double(inD) * mutation) <= 10.0)) ||
                    ((inD >= 20) && ((mutation) <= 0.05)))//condition for Poisson Approx
                {
    ...
    do {
        ...
        // cout<<"before\n";
        onegeneration(popN0, popN1, roundN, inD, gInd, selectionCo, gameNum, prob, probFNOPT, gamma, totalC0, totalC1,
                      totalE0, totalE1, genC0, genC1, outOpt, randRound, rng, uniR, uni2);
        //  cout<<"after\n";
        if (outOpt.find("1", 0) != string::npos) {
            ...
            for (int oneinD = 0; oneinD < inD; oneinD++) {
                ...
                if ((popN0[oneinD].GetInfo(2) > 0) && (popN0[oneinD].GetInfo(3) > 0)) {
                    totalC0[2] += (popN0[oneinD].GetInfo(3) /
                                   (popN0[oneinD].GetInfo(2) * popN0[oneinD].GetOrigEndowment()));
                }//get average payoff
                ...
                //cout<<popN[oneinD].GetInfo(2)<<"\n";
                if ((popN1[oneinD].GetInfo(2) > 0) && (popN1[oneinD].GetInfo(3) > 0)) {
                    totalC1[2] += (popN1[oneinD].GetInfo(3) /
                                   (popN1[oneinD].GetInfo(2) * popN1[oneinD].GetOrigEndowment()));
                }//get average payoff
                if (popN1[oneinD].GetInfo(2) > 0) {
                    totalC1[3] += (popN1[oneinD].GetGoalReached() / (popN1[oneinD].GetInfo(2)));
                }//get goal Reached
                //this collects the frequency of the 4 different strategies C=0, C<R, C=R,C>R
...
//    //for all new games the amount payed goes back to 0
    ...
    if (randRound == -2)
        oneRandRound = uni2(
                rng); //oneRandRound=RNUm.GetRand(0, roundN); //for each new game this is random but it is the same for one game and only happens once
    ...
    for (int r = 0; r < roundN; r++) {
        ...
        if (inD > 0) {
            for (int i = 0; i < gInd; i++) {
                ...
                if (num > (popN0[group0[i]].GetEndowment())) { num = (popN0[group0[i]].GetEndowment()); }

                //  cout<<group0[i]<<"-i-"; cout<<popN0[group0[i]].GetEndowment()<<"-"<<popN0[group0[i]].GetOrigEndowment()<<"-";cout<<num<<" ";

                popN0[group0[i]].SetBehavior(r, (popN0[group0[i]].GetBehavior(r) +
                                                 num));//total individual investement per round for all games
                //  cout<<num<<" ";
                ...
                if (num > (popN1[group1[i]].GetEndowment())) { num = (popN1[group1[i]].GetEndowment()); }

                //  cout<<group0[i]<<"-i-"; cout<<popN0[group0[i]].GetEndowment()<<"-"<<popN0[group0[i]].GetOrigEndowment()<<"-";cout<<num<<" ";

                popN1[group1[i]].SetBehavior(r, (popN1[group1[i]].GetBehavior(r) +
                                                 num));//total individual investement per round for all games
                //  cout<<num<<" ";
...
//                //cout<<probOPT[i]<<" "<< prob[i]<<" "<< target<<" "<< totalA<<" "<< gamma[i]<<" "<< double(gInd[0])<<"\n";
            ...
            if (uniR(rng) <= probOutput) { numP[0] = 1.0; }
...
//                { numP[0]=1.0;}
        ...
        if (inD > 0) {
            //cout<<"in6\n";
            for (int i = 0; i < gInd; i++) {
                popN0[group0[i]].SetEndowment(
                        popN0[group0[i]].GetEndowment() * (1.0 - popN0[group0[i]].GetLostEndowment() * numP[0]));
                if (outOpt.find("3", 0) != string::npos) totalE0[r] += popN0[group0[i]].GetEndowment();

                popN1[group1[i]].SetEndowment(
                        popN1[group1[i]].GetEndowment() * (1.0 - popN1[group1[i]].GetLostEndowment() * numP[0]));
...
void onegeneration(std::vector <Individual> &popN0, std::vector <Individual> &popN1, int const &roundN, int const &inD,
    ...
    for (int i = 0; i < gameNum; i++) {
        onegame(popN0, popN1, roundN, inD, gInd, prob, probOPT, gamma, totalC0, totalC1, totalE0, totalE1, genC0, genC1,
                outOpt, randRound, rng, uniR, uni2);
    }
